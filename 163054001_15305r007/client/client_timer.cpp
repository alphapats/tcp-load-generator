...
//block until all threads complete 
  ...
  free(vptr); 
   ...
   // Record end time
	...
	cout<<"Total MYSQL login  error" <<login_unsucc_counter<<endl;
  	...
  	result_file.open ("result.txt", ios::out | ios::app );
  	result_file<<endl<<NUM_THREADS<<"\t \t"<<data_exch<<"\t \t"<<mean_resp_time<<"\t \t"<<throughput<<"\t \t"<<complete_counter
  	<<"\t \t"<<error_counter<<"\t \t"<<RUN_TIME<<"\t \t"<<total_data_tfred<<"\t \t"<<elapsed.count()<<"\t \t"<<REQUEST_TYPE;
